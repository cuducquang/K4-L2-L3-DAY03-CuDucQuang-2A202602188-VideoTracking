...
Phần này ghi **trạng thái bản làm cá nhân ngày 15/09/2026**, không thay thế các
checkpoint bắt buộc phía trên. Repo nộp là:

```text
https://github.com/cuducquang/K4-L2-L3-DAY03-CuDucQuang-2A202602188-VideoTracking.git
```

Notebook dùng `ROOT=/content/Day3-Lab`. Clip core có 190 JPEG, 960×540,
12.5 fps; warm-up có 60 JPEG. Người học đã chạy Colab GPU và tải MOT/config
về máy; lần chạy notebook đó in `HAS_GOLD=False` vì ZIP chưa được giải nén
vào **đúng** `/content/Day3-Lab/gold/clip_01/gt.txt`. Vì vậy hai JSON so với
gold dưới đây được tính lại **local** từ MOT Colab và reference local, không
phải output đã chấm gold trong runtime Colab.

| File / output | Trạng thái và nguồn |
| --- | --- | --- |
